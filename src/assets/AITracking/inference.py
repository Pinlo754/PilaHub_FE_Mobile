...
class PoseCorrectionModel:
    def __init__(self, model_path, metadata_path, mappings_path):
        ...
        # Detect input order from metadata
        self.exercise_input_idx = 0 if self.metadata['inputs'][0]['type'] == 'exercise_input' else 1
        self.keypoints_input_idx = 1 - self.exercise_input_idx
        
        # Detect output order from metadata
        self.label_output_idx = 1
        self.body_part_output_idx = 0
        self.side_output_idx = 2
