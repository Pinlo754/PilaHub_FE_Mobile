...
import { getProducts, ProductItem, PagedResult, normalizeImageUrl } from '../../services/products';
...
// SearchResult (renamed from ResultsScreen) - modern search results with filters
export default function SearchResult() {
  ...
  const onAdd = async (item: ProductItem) => {
    try {
      await addToCart({ product_id: item.product_id, product_name: item.product_name, thumnail_url: normalizeImageUrl(item.thumnail_url), price: item.price, raw: item.raw }, 1);
  ...
  const renderCard = ({ item }: { item: ProductItem }) => (
    <TouchableOpacity onPress={() => navigation.navigate('ProductDetail', { productId: item.product_id })} style={[styles.card, { width: CARD_WIDTH }]}>
      <View style={styles.innerCard}>
        <Image source={{ uri: normalizeImageUrl(item.thumnail_url) }} style={styles.thumb} />
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <FlatList
          ...
          onEndReached={() => { if (items.length < total && !loadingMore) load(page + 1); }}
          onEndReachedThreshold={0.6}
          showsVerticalScrollIndicator={false}
          contentContainerStyle={styles.listContent}
          ListEmptyComponent={<View style={styles.emptyWrap}><Text style={styles.emptyText}>Không có kết quả</Text></View>}
        />
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity onPress={applyCustomFilter} style={styles.applyBtn}><Text style={styles.applyBtnText}>Áp dụng</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
