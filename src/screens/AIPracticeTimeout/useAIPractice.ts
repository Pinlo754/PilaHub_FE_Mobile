...
type Props = {
  route: RouteProp<RootStackParamList, 'AIPracticeTimeout'>;
};

export const useAIPracticeTimeout = ({ route }: Props) => {
  // CONSTANTS
  const TIMEOUT = 3000;

  // PARAM
  const { exercise_id, imgUrl, videoUrl, workoutSessionId, nameAITracking, timeout } = route.params;
