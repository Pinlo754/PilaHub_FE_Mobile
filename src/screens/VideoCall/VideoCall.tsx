...
const VideoCall = (props: Props) => {
  // HOOK
  const videoHook = useVideoCall({ navigation: props.navigation, route: props.route }) as any;
  const {
    cameraOn,
    isLoading,
    onLeave,
  ...
  const [coachHeartRate, setCoachHeartRate] = React.useState<number | null>(null);

  // Watch for heart rate updates from STOMP subscription
  React.useEffect(() => {
    if (latestHeartRate?.heartRate) {
      console.log('[VideoCall] ❤️ UPDATING UI with heart rate:', latestHeartRate.heartRate);
      console.log('[VideoCall] ❤️ latestHeartRate full object:', latestHeartRate);
      console.log('[VideoCall] ❤️ Setting coachHeartRate state to:', latestHeartRate.heartRate);
      setCoachHeartRate(latestHeartRate.heartRate);
      console.log('[VideoCall] ❤️ coachHeartRate state updated');
    } else {
      console.log('[VideoCall] ⚠️ latestHeartRate is null or empty:', latestHeartRate);
    }
  }, [latestHeartRate]);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const id = (await AsyncStorage.getItem('accountId')) || (await AsyncStorage.getItem('id')) || null;
        if (!mounted) return;
        setRole(storedRole);
        ...
        const base = api?.defaults?.baseURL ? String(api.defaults.baseURL).replace(/\/api\/?$/, '') : 'http://localhost:8080';
        ...
        if (storedRole === 'COACH' && id) {
          ...
          console.log('[VideoCall] COACH subscribing to heart rate, coachId:', id);
          unsub = subscribeToCoach(id, (payload: any) => {
            // latestHeartRate state updated by hook; set subscribed true on first message
            try {
              console.log('[VideoCall] 🎯 COACH RECEIVED HEART RATE PAYLOAD:', payload);
          ...
          console.log('[VideoCall] COACH subscription setup complete, unsub function exists:', typeof unsub === 'function');
        }
      } catch (e) {
        // ignore
      }
  ...
  const handleBleHr = React.useCallback((bpm: number) => {
    ...
    <>
      ...
      <View style={styles.hrOverlay}>
        <View style={styles.hrCard}>
          ...
          <Text style={{ color: '#fff', fontSize: 11, marginTop: 4 }}>{`STOMP: ${hrIsConnected ? 'connected' : 'disconnected'} • subscribed: ${hrSubscribed ? 'yes' : 'no'}`}</Text>
        </View>
      </View>
    ...
    <>
      ...
      <View style={styles.localVideoContainer}>
        ...
        <View style={styles.bleContainer}>
          <PolarHeartRate compact autoStart onHeartRate={(bpm) => handleBleHr(bpm)} />
          <View style={{ marginTop: 8, alignItems: 'center' }}>
            <Text style={{ color: '#fff', fontSize: 14 }}>{traineeBpm ? `${traineeBpm} bpm` : 'No HR'}</Text>
            <Text style={{ color: '#fff', fontSize: 12, marginTop: 6 }}>{lastSentAt ? `Sent ${(new Date(lastSentAt)).toLocaleTimeString()}` : 'Not sent yet'}</Text>
            <Text style={{ color: '#fff', fontSize: 11, marginTop: 6 }}>Auto-send enabled</Text>
          </View>
        </View>
      </View>
    </>
    ...
    <>
      ...
      <View className="flex-1 pt-14 bg-background">
        ...
        {((props.route.params as any)?.debug) && (
          <View style={{ position: 'absolute', top: 44, left: 12, zIndex: 999 }}>
            <View style={{ backgroundColor: 'rgba(0,0,0,0.7)', padding: 8, borderRadius: 6 }}>
              <Text style={{ color: '#fff', fontSize: 12 }}>channel: {debugInfo?.channelName ?? liveSessionDetail?.channelName ?? 'n/a'}</Text>
              <Text style={{ color: '#fff', fontSize: 12 }}>uid: {String(debugInfo?.uid ?? liveSessionDetail?.traineeUid ?? '')}</Text>
              <Text style={{ color: '#fff', fontSize: 10 }} numberOfLines={1}>token: {debugInfo?.token ? 'yes' : (liveSessionDetail?.traineeToken ? 'yes' : 'no')}</Text>
            </View>
          </View>
        )}
