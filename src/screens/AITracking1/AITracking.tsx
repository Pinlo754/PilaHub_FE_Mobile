...
import { LABELS, SCALER_MEAN, SCALER_SCALE } from '../../hooks/poseModel1';
...
export default function AITracking1({ workoutSessionId, onFeedback, captureMistakeImage, nameAITracking }: Props) {
  ...
  const handlePose = useCallback(
    async (data: any) => {
      ...
      try {
        ...
        console.log('Exercise name for inference:', exerciseName);
        ...
        if (exIdx !== -1) {
          ...
          console.error('CẢNH BÁO: Không tìm thấy bài tập', exerciseName, 'trong LABELS');
        }

        const outputs = await model.run([exArray, kpArray]);

        if (!outputs || outputs.length < 3) {
          console.error('Lỗi: Model không trả về đủ 3 outputs', outputs);
          return;
        }
        ...
        if (incorrectProb < 0.2) {
          ...
          const finalBodyPart = LABELS.body_parts[partIdx];
          const sideName = LABELS.sides[sideIdx];

          if (finalBodyPart === 'none') {
            // Nếu model trả về 'none', coi như động tác chuẩn
            if (activeMistake.current) {
              handleCorrect();
              return;
            }
            ...
            handleIncorrect(finalBodyPart, sideName);
          }
        }
      } catch (err) {
        console.error(err);
    ...
    [model, isSessionActive, nameAITracking],
  );
  ...
  const handleIncorrect = (bodyPart: string, side: string) => {
    ...
    if (now - pendingMistake.current.detectedAt >= DEBOUNCE_TIME) {
      ...
      play(bodyPart.toLowerCase().replace(/\s+/g, ''));
